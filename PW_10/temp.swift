/* Создать класс «бонусные акции» с полями название и бонус, 
...
class Bonus {
    ...
    func bonusApplicationClosure() -> (inout Client) -> Void {
        return { client in            
            let oldBalance = client.bonusBalance
            client.bonusBalance += self.bonusValue
            print("Начислено \(self.bonusValue) бонусов за акцию '\(self.title)'. Баланс: \(oldBalance) → \(client.bonusBalance)")
        }
    }
}
